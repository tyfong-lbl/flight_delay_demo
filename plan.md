...
- **Reproducibility controls:** temporal train/test split on `FL_DATE` with cutoff `2023-11-01` (train: Jan-Oct, test: Nov-Dec), `random_state=42` for model training, deterministic sampling seed
...
- **Deviation from spec:** The spec calls for a random stratified 80/20 split. The plan uses a **temporal split** (train on Jan-Oct 2023, test on Nov-Dec 2023) to avoid data leakage from future time periods, which is more realistic for flight delay prediction. The cutoff is `FL_DATE < 2023-11-01`.
...
4. **Add modeling utilities to `helpers.py`**
   - Temporal train/test split (split on `FL_DATE < 2023-11-01` for train, `>= 2023-11-01` for test).
